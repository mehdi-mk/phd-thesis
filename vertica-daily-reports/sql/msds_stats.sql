\timing
WITH summary AS (
SELECT V6_NTOA(resp_h) AS Resp_IP, COUNT(*) AS Conns, SUM(resp_bytes2) AS Outbound, SUM(orig_bytes2) AS Inbound, SUM(orig_bytes2 + resp_bytes2) AS Total_Bytes, COUNT(DISTINCT orig_h) AS Distinct_Hosts FROM daily_report_conn WHERE resp_p=445 GROUP BY Resp_IP
), 
conn_tot AS (
SELECT count(*) AS Total FROM daily_report_conn WHERE resp_p=445
),
byte_tot AS (
SELECT sum(orig_bytes2 + resp_bytes2) AS Total FROM daily_report_conn WHERE resp_p=445
)
