...
WHERE ((isUofCAddress2021(resp_h) AND orig_p >= resp_p AND NOT isUofCAddress2021(orig_h)) OR (isUofCAddress2021(orig_h) AND resp_p > orig_p AND NOT isUofCAddress2021(resp_h))) AND orig_bytes2 > 0 AND resp_bytes2 > 0 AND NOT (state='S0' OR state='REJ' OR (state = 'RSTOS0' AND history NOT LIKE '%d%'))
GROUP BY 1,2
),
conn_tot AS (
